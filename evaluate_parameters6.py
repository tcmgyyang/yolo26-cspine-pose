...
def compute_icc(ya, ym):
    ...
    Uses pingouin for validated computation with correct CI.
    Fallback to manual calculation if pingouin unavailable.
    """
    n = len(ya)
    if n < 3:
    ...
    try:
        ...
        # pingouin expects long-format DataFrame
        df_icc = pd.DataFrame({
            'targets': list(range(n)) * 2,
            'raters': ['manual'] * n + ['auto'] * n,
            'ratings': list(ym) + list(ya),
        })
        icc_df = pg.intraclass_corr(data=df_icc, targets='targets',
                                     raters='raters', ratings='ratings')
        # ICC2 = two-way random, absolute agreement, single measures
        row = icc_df[icc_df['Type'] == 'ICC2']
        if len(row) == 0:
            row = icc_df.iloc[1:2]  # ICC2 is the 2nd row (index 1)
        icc_val = float(row['ICC'].values[0])
        ci = row[['CI95%']].values[0][0]
        return icc_val, float(ci[0]), float(ci[1])
    except ImportError:
        # Fallback: manual ICC(2,1) with corrected CI
        ...
        # Corrected CI for ICC(2,1) per Shrout & Fleiss (1979)
        # Using F-test on modified ratio that accounts for MSC
        a = k * icc / (n * (1 - icc))
        b = 1 + k * icc * (n - 1) / (n * (1 - icc))
        v = (a * msc + b * mse) ** 2 / ((a * msc) ** 2 / (k - 1) + (b * mse) ** 2 / ((n - 1) * (k - 1)))

        F_val = msr / (a * msc + b * mse) if (a * msc + b * mse) > 1e-12 else 1e6
        F_L = F_val / stats.f.ppf(0.975, n - 1, max(v, 1))
        F_U = F_val * stats.f.ppf(0.975, max(v, 1), n - 1)

        lo = n * (F_L - 1) / (k + n * (F_L - 1))
        hi = n * (F_U - 1) / (k + n * (F_U - 1))
