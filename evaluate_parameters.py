...
- Publication-quality figures grouped by clinical category:
    Fig1: Alignment (Cobb, Ishihara Index, cSVA, Vertebral Slip)
    Fig2: Morphological (H/D Ratio, DHI, Canal Diameter)
    Fig3: Dynamic (Seg ROM, Total Cervical ROM, Seg Trans, ISD)
...
def calc_ishihara_index(k):
    """Ishihara Cervical Curvature Index: (a3+a4+a5+a6)/D × 100%"""
    p2,p7=pt(k,3),pt(k,32); D=edist(p2,p7)
    if D<1e-6: return 0.0
    return sum(perp_dist(pt(k,i),p2,p7) for i in [8,14,20,26])/D*100
...
def calc_hd_ratio(k):
    """Height-to-Depth Ratio: (Ha+Hm+Hp)/3 / VBd
    Ha = dist(AS, AI), Hm = dist(mid(AS,PS), mid(AI,PI)), Hp = dist(PS, PI)
    VBd = dist(AI, PI) — inferior endplate AP diameter
    """
    rs=[]
    for v in ['C3','C4','C5','C6','C7']:
        d=VERTEBRAE[v]
        Ha=edist(pt(k,d['AS']),pt(k,d['AI']))
        Hm=edist(mid(pt(k,d['AS']),pt(k,d['PS'])),mid(pt(k,d['AI']),pt(k,d['PI'])))
        Hp=edist(pt(k,d['PS']),pt(k,d['PI']))
        VBd=edist(pt(k,d['AI']),pt(k,d['PI']))
        if VBd>1e-6: rs.append((Ha+Hm+Hp)/3.0/VBd)
...
def calc_dhi(k):
    """Disc Height Index: (Da+Dp)/2 / dist(Cn_AI, Cn_PI) — denominator is upper vertebra AP diameter"""
    ...
    for u,l in segs:
        ...
        VBd=edist(pt(k,U['AI']),pt(k,U['PI']))  # upper vertebra inferior endplate AP diameter
        if VBd>1e-6: ds.append((Da+Dp)/2.0/VBd)
...
STATIC_PARAMS = [
    ...
    ('Ishihara_Index',calc_ishihara_index),
    ('cSVA',calc_csva),
    ('Vertebral_Slip',calc_slip),
    ('Height_Depth_Ratio',calc_hd_ratio),
...
def calc_seg_trans(kf,ke):
    """Segmental Translation: |[x(Cn_PI)-x(Cn+1_PS)]_flex - [x(Cn_PI)-x(Cn+1_PS)]_ext|"""
    segs=[(3,7),(8,13),(14,19),(20,25),(26,31)]  # (Cn_PI, Cn+1_PS)
    trans=[]
    for pi_idx, ps_idx in segs:
        df = pt(kf,pi_idx)[0] - pt(kf,ps_idx)[0]  # relative displacement in flexion
        de = pt(ke,pi_idx)[0] - pt(ke,ps_idx)[0]   # relative displacement in extension
        trans.append(abs(df - de))
    return max(trans) if trans else 0.0
...
DYNAMIC_PARAMS = [
    ...
    ('Total_Cervical_ROM',calc_global_rom),
    ('Segmental_Translation',calc_seg_trans),
    ('Interspinous_Distance',calc_isd_change),
]
...
PARAM_CATEGORIES = {
    'Alignment Parameters': {
        'color': CLR['cat1'],
        'keys': ['Cobb_Angle','Ishihara_Index','cSVA','Vertebral_Slip'],
        'display': {
            'Cobb_Angle': 'Cobb Angle (°)',
            'Ishihara_Index': 'Ishihara Index (%)',
            'cSVA': 'cSVA (pixels)',
    ...
    'Morphological Parameters': {
        'color': CLR['cat2'],
        'keys': ['Height_Depth_Ratio','Disc_Height_Index','Spinal_Canal_Diameter'],
        'display': {
            'Height_Depth_Ratio': 'Height-to-Depth Ratio',
    ...
    'Dynamic Parameters': {
        'color': CLR['cat3'],
        'keys': ['Segmental_ROM','Total_Cervical_ROM','Segmental_Translation','Interspinous_Distance'],
        'display': {
            'Segmental_ROM': 'Segmental ROM (°)',
            'Total_Cervical_ROM': 'Total Cervical ROM (°)',
            'Segmental_Translation': 'Segmental Translation (pixels)',
            'Interspinous_Distance': 'Interspinous Distance (pixels)',
        },
    },
}
...
def compute_icc(ya, ym):
    """ICC(2,1): Two-way random, absolute agreement, single measures + 95% CI."""
    n = len(ya)
    if n < 3:
    ...
    try:
        ...
        df_icc = pd.DataFrame({
            ...
            'ratings': list(ym) + list(ya),
        })
        result = pg.intraclass_corr(data=df_icc, targets='targets',
                                     raters='raters', ratings='ratings')
        row = result[result['Type'] == 'ICC2'].iloc[0]
        icc_val = float(row['ICC'])
        ci = row['CI95%']  # numpy array [lower, upper]
        return icc_val, float(ci[0]), float(ci[1])
    except Exception:
        # Manual fallback: ICC(3,1) which has reliable CI via F-transform
        # ICC(3,1) = two-way mixed, consistency, single measures
        data = np.column_stack([ym, ya]); k = 2
        gm = np.mean(data); rm = np.mean(data, 1)
        sst = np.sum((data - gm) ** 2)
        ssr = k * np.sum((rm - gm) ** 2)
        ssc = n * np.sum((np.mean(data, 0) - gm) ** 2)
        sse = sst - ssr - ssc
        msr = ssr / (n - 1)
        mse = sse / ((n - 1) * (k - 1)) if (n - 1) * (k - 1) > 0 else 1e-12
...
def generate_forest_plot(summary, plots_dir):
    ...
    DISPLAY = {
        ...
        'Interspinous_Distance': 'Interspinous Distance',
    }
    CAT_MAP = {
        'Cobb_Angle': 'Alignment',
        'Ishihara_Index': 'Alignment',
        'cSVA': 'Alignment',
        'Vertebral_Slip': 'Alignment',
        'Height_Depth_Ratio': 'Morphological',
        'Disc_Height_Index': 'Morphological',
        'Spinal_Canal_Diameter': 'Morphological',
        'Segmental_ROM': 'Dynamic',
        'Total_Cervical_ROM': 'Dynamic',
        'Segmental_Translation': 'Dynamic',
        'Interspinous_Distance': 'Dynamic',
    }
    CAT_COLORS = {
        'Alignment': CLR['cat1'],
        'Morphological': CLR['cat2'],
        'Dynamic': CLR['cat3'],
    }

    # Ordered param list
    ORDER = ['Cobb_Angle','Ishihara_Index','cSVA','Vertebral_Slip',
             'Height_Depth_Ratio','Disc_Height_Index','Spinal_Canal_Diameter',
             'Segmental_ROM','Total_Cervical_ROM','Segmental_Translation','Interspinous_Distance']
...
def main():
    ...
    print(f"  plots/Fig1_Alignment_Parameters.png/.pdf")
    print(f"  plots/Fig2_Morphological_Parameters.png/.pdf")
    print(f"  plots/Fig3_Dynamic_Parameters.png/.pdf")
