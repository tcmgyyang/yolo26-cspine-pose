...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c1 = ch[f]
            c2 = c1 * (args[0] ** 2) if len(args) > 1 and args[1] == 'pl' else c1  # pl模式通道数变化
            c2 = c1  # DySample 输出通道数等于输入
            args = [c1, *args]  # 添加输入通道数
        elif m is LAWDS:
            c1 = ch[f]
            c2 = c1  # LAWDS output channels = input channels
            args = [c1]
        elif m is FocusFeature:
            # FocusFeature 接收3个不同尺度的特征图
            e = args[1] if len(args) > 1 else 0.5
            c2 = int(ch[f[1]] * e) * 3  # hidc * 3
            args = [[ch[f[0]], ch[f[1]], ch[f[2]]]] + args
        elif m is SBA:
            c2 = args[0] if args else 64
            args = [[ch[f[0]], ch[f[1]]], c2]
        elif m in {EfficientChannelAttention, DPCA}:
            c1 = ch[f]
            c2 = c1  # ECA 不改变通道数
            args = [c1, *args]  # 添加输入通道数
        else:
