...
def get_label_path(img_path):
    ...
    for ext in ['.jpg','.jpeg','.png','.bmp','.tif','.tiff']:
        while lp.lower().endswith(ext): lp=lp[:-len(ext)]
    return lp+'.txt'
...
# ============================================================
def scan_all(dirs):
    patients=defaultdict(dict); laterals=[]
    exts={'.jpg','.jpeg','.png','.bmp','.tif','.tiff'}
    for d in dirs:
        if not os.path.isdir(d): continue
        for f in os.listdir(d):
            if not any(f.lower().endswith(e) for e in exts): continue
            full=os.path.join(d,f); name=f
            for ext in ['.jpg','.jpeg','.png','.bmp','.tif','.tiff']:
                while name.lower().endswith(ext): name=name[:-len(ext)]
...
def main():
    ...
    print("=" * 70)
    laterals, pairs = scan_all(SCAN_DIRS)
    print(f"  Lateral images: {len(laterals)}")
    print(f"  Flex-ext pairs: {len(pairs)}")
    ...
    print("\n" + "=" * 70)
    print(f"Step 2: Static parameters (n={len(laterals)})")
    print("=" * 70)
    sres = []; stimes = []
    for ip in sorted(laterals):
    ...
    # ============================================================
    print("\n" + "=" * 70)
    print("Step 5: Generating publication figures (3 categories)")
    print("=" * 70)
