"""
Batch model validation for the DPCA-placement ablation series.

Validates every DPCA configuration's `best.pt` on the test split and emits
the per-model + comparison tables used in the paper.
"""
...
# Configuration
# ============================================================
BASE_DIR = 'runs/pose/spine_extreme/DPCA'
DATA_YAML = 'dataset/data.yaml'
OUTPUT_DIR = 'runs/pose/spine_extreme/DPCA/validation_results'

# Run-directory names for every DPCA configuration to validate
DPCA_MODELS = [
    # Single position
    'DPCA_A_sigma',
    'DPCA_B_sigma',
...
def validate_single_model(model_name, base_dir, data_yaml, output_dir):
    """Validate one model run directory and return its metrics dictionary."""
...
def generate_comparison_report(all_results, output_dir):
    """Emit the cross-model comparison report."""
    if not all_results:
    ...
    info_table.title = "DPCA Ablation - Basic Info Comparison"
    ...
    box_table.title = "DPCA Ablation - Box Detection Metrics"
    ...
    pose_table.title = "DPCA Ablation - Pose Estimation Metrics"
    ...
    print("\n" + "=" * 80)
    print("DPCA ablation — cross-model comparison report")
    print("=" * 80)
    print(info_table)
    print(box_table)
    print(pose_table)
    ...
    report_path = os.path.join(output_dir, 'DPCA_ablation_comparison.txt')
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write(f"DPCA ablation comparison report — generated {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write("=" * 80 + "\n\n")
    ...
    csv_path = os.path.join(output_dir, 'DPCA_ablation_comparison.csv')
    with open(csv_path, 'w', encoding='utf-8') as f:
        f.write("Model,GFLOPs,Params(M),FPS,Size(MB),")
        ...
        for r in all_results:
            f.write(f"{r['model_name']},{r['gflops']:.1f},{r['parameters']/1e6:.2f},")
...
if __name__ == '__main__':
    print("=" * 60)
    print("Batch validation — DPCA ablation series")
    print(f"Start: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)

    # Check which model directories actually exist
    valid_models = []
    for model_name in DPCA_MODELS:
        model_path = os.path.join(BASE_DIR, model_name, 'weights', 'best.pt')
        if os.path.exists(model_path):
            valid_models.append(model_name)
        else:
            print(f"[warn] not found: {model_name}/weights/best.pt")

    print(f"\nFound {len(valid_models)} valid models to validate:")
    for i, name in enumerate(valid_models, 1):
        print(f"  {i}. {name}")
    ...
    print(f"Validated successfully: {len(all_results)}/{len(valid_models)} models")
